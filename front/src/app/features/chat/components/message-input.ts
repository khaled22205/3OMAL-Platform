  ...
  template: `
    <div class="p-4 bg-white dark:bg-slate-950 border-t border-slate-100 dark:border-slate-850">
      @if (store.replyToMessage(); as reply) {
        <div class="flex items-center justify-between px-3 py-2 mb-2 bg-slate-50 dark:bg-slate-900 rounded-lg text-right">
          <div class="flex-grow">
            <span class="text-[10px] font-bold text-primary block">الرد على {{ reply.senderName }}</span>
            <span class="text-[10px] text-slate-500">{{ (reply.content || 'مرفق') | slice:0:40 }}</span>
          </div>
          <button (click)="store.setReplyTo(null)" class="text-red-500 text-xs font-bold cursor-pointer">&times;</button>
        </div>
      }

...
@Component({
  ...
  template: `
    <div
      class="min-h-[90vh] flex items-center justify-center px-4 py-12 sm:px-6 lg:px-8 bg-slate-50 dark:bg-slate-900 transition-colors duration-300"
    >
      <div
        class="max-w-xl w-full space-y-8 glass p-8 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-800"
      >
        <!-- Header -->
        ...
        <form [formGroup]="form" (ngSubmit)="onSubmit()" class="mt-8 space-y-6">
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="space-y-1">
              ...
              <input
                ...
                placeholder="محمد"
                class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-white/50 dark:bg-slate-950/50 text-slate-800 dark:text-white outline-none focus:border-primary transition-colors text-right"
                [class.is-invalid]="
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div class="space-y-1">
              ...
              <input
                ...
                class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-white/50 dark:bg-slate-950/50 text-slate-800 dark:text-white outline-none focus:border-primary transition-colors text-right ltr"
                [class.is-invalid]="
          ...
          @if (form.errors?.['passwordMismatch'] && form.controls.confirmPassword.touched) {
            <small class="text-rose-500 text-xs font-bold pr-1 block -mt-4"
          ...
          <div class="flex p-1 bg-slate-100 dark:bg-slate-800 rounded-xl">
            <button
              type="button"
              (click)="form.controls.userType.setValue('Customer')"
              [class]="
                form.controls.userType.value === 'Customer'
                  ? 'w-1/2 py-3 bg-white dark:bg-slate-900 text-slate-900 dark:text-white shadow-md'
                  : 'w-1/2 py-3 text-slate-500 dark:text-slate-400 hover:text-slate-950'
              "
            ...
            <button
              type="button"
              (click)="form.controls.userType.setValue('Worker')"
              [class]="
                form.controls.userType.value === 'Worker'
                  ? 'w-1/2 py-3 bg-white dark:bg-slate-900 text-slate-900 dark:text-white shadow-md'
                  : 'w-1/2 py-3 text-slate-500 dark:text-slate-400 hover:text-slate-950'
              "
              class="rounded-lg font-bold text-sm transition-all cursor-pointer"
  ...
  onSubmit(): void {
    ...
    this.auth.register(request).subscribe({
      ...
      error: (err: HttpErrorResponse) => {
        ...
        if (!errorBody) {
          this.serverError =
            err.status === 0 ? 'خطأ في الشبكة. تحقق من اتصالك.' : `خطأ في الخادم (${err.status}).`;
          return;
        }
        ...
        if (errors && typeof errors === 'object' && !Array.isArray(errors)) {
          ...
          this.serverError =
            messages.length > 0
              ? messages.join('. ')
              : (errorBody['title'] as string) || 'فشل التحقق من البيانات.';
          return;
        }
        this.serverError =
          (errorBody['message'] as string) ||
          (errorBody['title'] as string) ||
          `فشل التسجيل (${err.status}).`;
      },
    });
  }
}

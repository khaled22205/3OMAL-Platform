...
@Component({
  ...
  template: `
    <div class="min-h-[80vh] flex items-center justify-center px-4 py-12 sm:px-6 lg:px-8 bg-slate-50 dark:bg-slate-900 transition-colors duration-300">
      <div class="max-w-md w-full space-y-8 glass p-8 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-800">
        ...
        <div class="text-center">
          ...
          <p class="mt-2 text-sm text-slate-500 dark:text-slate-400">
            أو 
            <a routerLink="/register" class="font-bold text-primary hover:text-primary-hover transition-colors">إنشاء حساب جديد</a>
          </p>
        </div>
        ...
        <form [formGroup]="form" (ngSubmit)="onSubmit()" class="mt-8 space-y-6">
          ...
          <!-- Email Field -->
          <div class="space-y-1">
            <label for="email" class="text-sm font-bold text-slate-700 dark:text-slate-350">البريد الإلكتروني</label>
            <input 
              id="email" 
              type="email" 
          ...
          <div class="space-y-1">
            <div class="flex justify-between items-center">
              <label for="password" class="text-sm font-bold text-slate-700 dark:text-slate-350">كلمة المرور</label>
            </div>
            <div class="relative">
              <input 
                id="password" 
              ...
              >
              <button type="button" (click)="togglePassword()" class="absolute left-3 top-1/2 -translate-y-1/2 text-xs font-bold text-slate-400 hover:text-primary cursor-pointer">
...
export default class Login {
  ...
  onSubmit(): void {
    ...
    this.auth.login({ email, password }).subscribe({
      ...
      error: (err) => {
        this.serverError = err.error?.message || err.error?.title || 'فشل تسجيل الدخول. حاول مرة أخرى.';
      },
    });
  }

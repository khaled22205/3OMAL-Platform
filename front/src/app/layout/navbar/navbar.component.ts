...
@Component({
  ...
  template: `
    <nav
      class="sticky top-0 z-50 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-100 dark:border-slate-800 transition-colors duration-300"
    >
        ...
        <div class="flex items-center justify-between h-16 sm:h-20">
          ...
          <div class="flex items-center gap-8">
            <a routerLink="/" class="flex items-center gap-2 group">
              <div
                class="w-10 h-10 rounded-xl bg-primary flex items-center justify-center text-white font-extrabold text-xl shadow-lg shadow-primary/20 transform group-hover:scale-105 transition-transform duration-300"
              >
                ع
              </div>
              <div class="flex flex-col">
                <span
                  class="text-xl sm:text-2xl font-black text-slate-850 dark:text-white tracking-tight group-hover:text-primary transition-colors"
                  >عمال</span
                >
                <span
                  class="text-[9px] font-bold text-slate-400 dark:text-slate-500 -mt-1 hidden sm:inline"
                  >أقرب صنايعي... في أسرع وقت.</span
                >
              </div>
            </a>
            ...
            <div class="hidden lg:flex items-center gap-6">
              <a
                ...
                class="text-sm font-semibold text-slate-600 dark:text-slate-300 hover:text-primary dark:hover:text-primary transition-colors"
                >الرئيسية</a
              >
              <a
                routerLink="/search"
            ...
            @if (authService.currentUser(); as user) {
              ...
              >
                ...
                @if (chatStore.unreadCount() > 0) {
                  <span
                    class="absolute -top-1 -left-1 min-w-[18px] h-[18px] bg-red-500 text-white text-[10px] font-extrabold rounded-full flex items-center justify-center px-1"
                  >
              ...
              <div class="flex items-center gap-3">
                ...
                >
                  ...
                  <div class="flex flex-col text-right">
                    ...
                    <span class="text-[10px] text-slate-400 font-semibold leading-none">{{
              ...
              <a
                routerLink="/login"
                class="text-sm font-bold text-slate-700 dark:text-slate-200 hover:text-primary dark:hover:text-primary transition-colors cursor-pointer"
                >تسجيل الدخول</a
              >
              <a
                routerLink="/register"
                class="px-5 py-2.5 text-sm font-bold bg-primary hover:bg-primary-hover text-white rounded-xl shadow-md shadow-primary/10 hover:shadow-lg transition-all cursor-pointer"
                >انضم كصنايعي</a
              >
            }
          </div>
      ...
      >
        <a
          ...
          class="text-base font-bold text-slate-700 dark:text-slate-200 py-2 border-b border-slate-50 dark:border-slate-800/50"
          >الرئيسية</a
        >
        <a
          routerLink="/search"
        ...
        @if (authService.currentUser(); as user) {
          <a
            [routerLink]="getDashboardRoute(user.role)"
            (click)="mobileMenuOpen.set(false)"
            class="flex items-center gap-3 py-2 border-b border-slate-50 dark:border-slate-800/50"
          >
          ...
          <a
            routerLink="/login"
            (click)="mobileMenuOpen.set(false)"
            class="text-center text-base font-bold text-slate-700 dark:text-slate-200 py-3 rounded-xl border border-slate-200 dark:border-slate-800"
            >تسجيل الدخول</a
          >
          <a
            routerLink="/register"
            (click)="mobileMenuOpen.set(false)"
            class="text-center text-base font-bold bg-primary hover:bg-primary-hover text-white py-3 rounded-xl shadow-md"
            >انضم كصنايعي</a
          >
        }
      </div>
